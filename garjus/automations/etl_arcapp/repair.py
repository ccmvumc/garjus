...
logger = logging.getLogger('garjus.automations.etl_arcapp.repair')
...
def _process(project, record_id, event_id):
    ...
    # Skip records that have complete test fields
    if len(rec) > 0 and len([x for x in rec if x['arc_order_index'] == '']) == 0:
