...
# -5 is an error code exported from PsychoPy and should be NA
data_raw[data_raw == -5] <- NA  

# input language that is not numeric and is written as text specifying English must be converted to numeric 1
data_raw$language <- tolower(data_raw$language)
data_raw$language[data_raw$language == 'english'] <- '1'
data_raw$language <- as.numeric(data_raw$language)
...
# Scores that are effectively 0 and 1 due to missing input data need to be set to NA
composite_scores_for_NA <- data_out[, c("executive_composite","executive_se","fluency_factor","fluency_se","cog_control_factor","cog_control_se","working_memory_factor","working_memory_se")]
composite_scores_for_NA[(composite_scores_for_NA <= 1.00000000001339 & composite_scores_for_NA >= 1.00000000001337) | composite_scores_for_NA == 0.0000000000] <- NA

data_out[, c("executive_composite","executive_se","fluency_factor","fluency_se","cog_control_factor","cog_control_se","working_memory_factor","working_memory_se")] <- composite_scores_for_NA
